...
export default function GalleryModal({
  ...
  const [category, setCategory] = useState(selectedImage?.category || "");

  const [event, setEvent] = useState(selectedImage?.event || "");

  const [image, setImage] = useState(selectedImage?.image || "");

  const [featured, setFeatured] = useState(selectedImage?.featured || false);

  const handleSubmit = () => {
    if (!title || !description || !category || !event || !image) {
      ...
      <div className="flex max-h-[90vh] w-full max-w-6xl flex-col overflow-hidden rounded-2xl bg-white shadow-2xl">
            ...
            <div className="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm">
              {image ? (
                ...
                <div className="flex h-80 flex-col items-center justify-center text-gray-400">
                  ...
                  <p className="mt-4 text-sm">Image preview will appear here</p>
                </div>
              )}
          ...
          <div className="space-y-5 p-8 lg:col-span-3">
            ...
            <div className="rounded-xl border border-gray-200 bg-gray-50 p-4">
              <label className="flex cursor-pointer items-center gap-3">
                ...
                <div>
                  <p className="font-medium text-gray-900">Featured Image</p>
        ...
        <div className="flex justify-end gap-3 border-t border-gray-200 bg-white px-8 py-5">
          ...
          <button
            ...
            "
          >
            {mode === "add" ? "Save Image" : "Update Image"}
          </button>
        </div>
      </div>
    </div>
